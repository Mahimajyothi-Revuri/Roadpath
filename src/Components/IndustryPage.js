...
function IndustryPage() {
  ...
  // Define the job lists for each industry
  const jobList = {
    "Education": [
  ...
  // Generate an alphabetically sorted array of industry names for the dropdown
  ...
  useEffect(() => {
    // Load saved industry and search term from localStorage
    ...
    // Update the job list when the industry changes; sort alphabetically
    if (jobList[industry]) {
      setJobs([...jobList[industry]].sort((a, b) => a.localeCompare(b)));
    } else {
      setJobs([...jobList['Education']].sort((a, b) => a.localeCompare(b))); // Fallback to 'Education'
    }
    
    // Reset selected jobs when industry changes
    setSelectedJobs([]);
  }, [industry]);
  ...
  // Toggle the selection of a job when clicked
  ...
  // Filter and sort the jobs alphabetically based on the search term
  const filteredJobs = jobs
    .filter(job => job.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => a.localeCompare(b));

  // Ensure the selected jobs are displayed in alphabetical order
          ...
          <div className='job-boxes'>
            {filteredJobs.length > 0 ? (
              filteredJobs.map((job, index) => (
                <div
                  key={index}
                  className={`job-box ${selectedJobs.includes(job) ? 'selected' : ''}`}
          ...
          <div>
            ...
            <div className='job-boxes'>
              {sortedSelectedJobs.map((job, index) => (
                <div key={index} className='job-box selected'>
                  <p>{job}</p>
                </div>
              ))}
            </div>
